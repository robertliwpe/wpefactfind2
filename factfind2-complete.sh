    ...
    do installloc="/nas/content/live/$i"; 
        if [ -d $installloc ]; 
            then 
                ...
                static=$(zcat -f /var/log/nginx/$i.access.log* | grep -v "jpg\|jpeg\|png\|svg\|gif\|webp\|woff\|woff2\|ttf\|otf\|xml\|css\|ico\|\.js\|txt\|pdf\|mov\|mp4\|mp3\|aiff\|mpg\|mpeg\|ogg" | wc -l); 
                dyn=$(zcat -f /var/log/apache2/$i.access.log* | wc -l); 
                comp=$(awk -v staticin=$static -v dynin=$dyn 'BEGIN { print staticin - dynin }');   
