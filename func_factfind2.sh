...
FLOCK_LOCK_FILE='/opt/redshell/flock.rsh_factfind'

function factfind2 {

# Check if factfind/factfind2 is running elsewhere and prevent double execution.

if ! flock -s --nonblock 9; then
  echo "Could not unlock ${FLOCK_LOCK_FILE}"
  echo "Factfind 2 is already running. Check ps faux"
else
            ...
            if [ -d $installloc ]; 
                then 
                    ...
                    while [[ $dbsizeinit == 0 ]]; do
                        dbsizeinit=$(echo $(wp db query --skip-plugins --skip-themes "SELECT SUM(round(((data_length + index_length) / 1024 / 1024) , 2)) FROM information_schema.TABLES;" | tail -2 | head -1 | column -s '|' -t | tr -d '\r') | bc 2>/dev/null);                        if [[ -z "$dbsizeinit" ]] 
                            then
                                dbsizeinit=0;
                            else
